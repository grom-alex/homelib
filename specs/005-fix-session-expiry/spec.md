...
### User Story 3 - Увеличение времени жизни сессии (Priority: P3)
...
**Why this priority**: Улучшение удобства использования. С увеличенным временем жизни токена пользователи реже будут сталкиваться с процессом обновления, что снижает нагрузку и улучшает отзывчивость.

**Independent Test**: Войти в систему, работать в течение 1.5 часов без перерыва. За это время не должно произойти ни одного обновления токена. После 2 часов первое действие должно вызвать прозрачное обновление.

**Acceptance Scenarios**:

1. **Given** пользователь только что авторизовался, **When** он работает с интерфейсом в течение 1 часа 59 минут, **Then** все запросы выполняются с оригинальным токеном без обновлений
2. **Given** прошло более 2 часов с момента последнего получения токена, **When** пользователь выполняет действие, **Then** система обновляет токен автоматически (поведение из US1)
...
- **FR-005**: Время жизни короткоживущего токена ДОЛЖНО составлять 2 часа
...
- **SC-001**: Пользователь может работать с приложением непрерывно в течение 2 часов без видимых прерываний или ошибок аутентификации
